...
if __name__ == '__main__':
    ...
    fold_test_data = "data/ER/d=10R=3.1/test"
    fold_model = "model/ER"
    fold_img = "data/ER/d=10R=3.1/img"

    text = []
    net = ["ER"]
    d = [10]
    # r = [1.2,1.5,1.8,2.0,2.2,2.5,3.0,3.3,3.5]
    # num = 5
    r = [3.1]
    num = 30
    ...
    if isV:
        label = "virosolver,mse="
        Vpre,_ = getViro(test_path ,Inum,pre_step=8)
        y_list_all.append(y_list)
        pred_list_all.append(Vpre)
        label_all.append(label)
        duration.append(duration_list)


    def keshi(y,pred,start,end,label,color,marker,linestyle):
        ...
        plt.plot(x,pred, c=color,marker=marker,markevery=10,linestyle=linestyle,lw=3,label=label_value)
    ...
    for x in range(len(y_list_all[0])):
        ...
        color = ["rosybrown","tomato","chocolate","goldenrod","y","green","lightseagreen","teal","deepskyblue","blue","blueviolet","m","deeppink"]
        marker = ["s","x","o","*"]
        linestyle = ["-.","dashed","dotted","dotted"]

        for y in range(len(y_list_all)):
            mse_value,mae_value = keshi(y_list_all[y][x],pred_list_all[y][x],duration[y][x][0],duration[y][x][1],label_all[y],color[y + 3],marker[y],linestyle[y])
        ...
        plt.grid(axis='y')
        plt.axhline(y=1)
        plt.title("net = " + str(valid_text[x][0]) + ",d = " + str(valid_text[x][1]) + ",R0 = " + str(valid_text[x][2]) + ",num = " + str(valid_text[x][3]),fontsize=14)
        plt.legend(fontsize=14)
        plt.xlabel('t',fontsize=14)
        plt.ylabel('Rt',fontsize=14)
        # plt.show()
        plt.savefig(fold_img + "/test_" + str(x) + ".png")
        plt.close()
