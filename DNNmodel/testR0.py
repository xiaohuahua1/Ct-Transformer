...
def predict(model_type,info,fold_model,fold_train_data,net_train,d_train,R_train,Inum,test_path,smooth,text):
    Dte = []
    y_list = []
    pred_list = []
    duration = []
...
if __name__ == '__main__':
    ...
    model_fold = "model/ER/loss/smooth"
    result_fold = "/result/ER/loss/smooth"
    ...
    for i in range(len(test_fold_list)):
        ...
        info = "smooth"
        y_list,pred_list,valid_text,duration_list = predict("myModel",info,model_fold,fold_train_data,net_train,
                                                            d_train,R_train,Inum,test_fold,smooth,text_num)
        
        Cf = open(result_fold_combine + "/Ct-Former_" + info + ".txt","w")
        Df = open(test_fold + "/result/duration.txt","w")
        Yf = open(test_fold + "/result/truth.txt","w")
        id = 0
        for j in range(total_num):
            if j not in valid_text:
                ...
                for z in range(len(pred)):
                    ...
                    Yf.write("{:.6f}".format(y[z]) + " ")
                Cf.write("\n")
                Yf.write("\n")
                
                duration = duration_list[id]
                Df.write(str(duration[0]) + " " + str(duration[1]) + "\n")
        ...
        csvCf = open(result_fold_combine + "/Ct-Former_" + info + ".csv","w",newline='')
        csvC_result = []
        ...
        # info = "best"
        y_list,pred_list,valid_text,duration_list = predict("TFT",info,model_fold,fold_train_data,net_train,
                                                            d_train,R_train,Inum,test_fold,smooth,text_num)
        
        Tf = open(result_fold_combine + "/TFT_" + info + ".txt","w")
        id = 0
        for j in range(total_num):
            if j not in valid_text:
                Tf.write('0 \n')
        ...
        csvTf = open(result_fold_combine + "/TFT_" + info + ".csv","w",newline='')
        csvT_result = []
        ...
        y_list,pred_list,valid_text,duration_list = predict("transformerEncoder",info,model_fold,fold_train_data,net_train,
                                                            d_train,R_train,Inum,test_fold,smooth,text_num)
        
        Ef = open(result_fold_combine + "/transformerEncoder_" + info + ".txt","w")
        id = 0
        for j in range(total_num):
            if j not in valid_text:
                Ef.write('0 \n')
        ...
        csvEf = open(result_fold_combine + "/transformerEncoder_" + info + ".csv","w",newline='')
        csvE_result = []
        ...
        # Regression
        if isR:
            Rpre,_ = getRegression(test_fold ,Inum,pre_step=8)
            Rf = open(test_fold + "/result/Regression.txt","w")
            id = 0
            for j in range(total_num):
                if j not in valid_text:
                    Rf.write('0 \n')
            ...
            csvRf = open(test_fold + "/result/Regression.csv","w",newline='')
            csvR_result = []
            csvR_head = ["id","MSE","MAE","RMSE","MAPE","R2"]
            csvR_result.append(csvR_head)

            id = 0
            for j in range(total_num):
                if j not in valid_text:
                    csvR_row = [str(j),"0","0","0","0","0","0"]
                else:
                    pred = Rpre[id]
                    y = y_list[id]
                    ...
                    csvR_row = [str(j),"{:.4f}".format(mse_value),"{:.4f}".format(mae_value),
                                "{:.4f}".format(rmse_value),"{:.4f}".format(mape_value),"{:.4f}".format(r2_value)]
                    
                    id += 1
                csvR_result.append(csvR_row)
            ...
            csvR_row = ["ave","{:.4f}".format(MSE_result),"{:.4f}".format(MAE_result),
                                "{:.4f}".format(RMSE_result),"{:.4f}".format(MAPE_result),"{:.4f}".format(R2_result)]
            
            csvR_result.append(csvR_row)

            writer = csv.writer(csvRf)
            writer.writerows(csvR_result)
            csvRf.close()

        

        if isB:
            Ts,M,U,L,R = getBayes(test_fold,Inum,pre_step=8)
            Bf = open(test_fold + "/result/Bayesian.txt","w")
            id = 0
            for j in range(total_num):
                if j not in valid_text:
                    Bf.write('0 \n')
            ...
            csvBf = open(test_fold + "/result/Bayesian.csv","w",newline='')
            csvB_result = []
            csvB_head = ["id","MSE","MAE","RMSE","MAPE","R2"]
            csvB_result.append(csvB_head)

            id = 0
            for j in range(total_num):
                if j not in valid_text:
                    csvB_row = [str(j),"0","0","0","0","0","0"]
                else:
                    pred = M[id]
                    y = y_list[id]
                    ...
                    csvB_row = [str(j),"{:.4f}".format(mse_value),"{:.4f}".format(mae_value),
                                "{:.4f}".format(rmse_value),"{:.4f}".format(mape_value),"{:.4f}".format(r2_value)]
                    
                    id += 1
                csvB_result.append(csvB_row)
            ...
            csvB_row = ["ave","{:.4f}".format(MSE_result),"{:.4f}".format(MAE_result),
                                "{:.4f}".format(RMSE_result),"{:.4f}".format(MAPE_result),"{:.4f}".format(R2_result)]
            
            csvB_result.append(csvB_row)

            writer = csv.writer(csvBf)
            writer.writerows(csvB_result)
            csvBf.close()
