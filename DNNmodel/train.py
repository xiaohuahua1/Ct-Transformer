...
if __name__ == '__main__':
    ...
    def train(model_type,info):
        ...
        if model_type == "transformerEncoder":
            path = fold_model + "/transformerEncoder_" + info + ".pkl"
            ...
            #             trainModel(args,model,Dtr_all[count],Dva_all[count],path,net[i],d[j],R[z])
            #             count += 1
            trainModel(args,model,Dtr,Dva,path)
        elif model_type == "TFT":
            path = fold_model + "/TFT_" + info + ".pkl"
            ...
            path = fold_model + "/myModel_" + info + ".pkl"
            args = CtTransformer_args_parser()
            Dtr,Dva,Dtr_all,Dva_all = get_Train_range(args,fold_data,net,d,R,Inum,smooth,max_list,min_list,max_rt,min_rt)
