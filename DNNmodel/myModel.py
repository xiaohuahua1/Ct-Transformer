...
class PretrainHead(nn.Module):
    def __init__(self, d_model, patch_len, input_size,output_size,dropout):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.linear = nn.Linear(d_model, patch_len*input_size*output_size)
    ...
    def forward(self, x):
        ...
        x = x.view(batch,patch_num,self.patch_len*self.input_size,-1)
...
class PredictHead(nn.Module):
    def __init__(self, d_model, patch_len, output_size,dropout):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.embed = nn.Linear(d_model,4*d_model)
        self.linear = nn.Linear(4*d_model, patch_len*output_size)
...
#                 torch.nn.init.kaiming_normal_(p, a=0, mode='fan_in', nonlinearity='sigmoid')
    ...
    def build_variable_selection_networks(self):
        ...
        self.temporal_future_vsn = VariableSelectionNetwork(hidden_layer_size = self.hidden_layer_size,input_size = self.hidden_layer_size *self.input_size*self.patchLen,
                                                            output_size = self.input_size*self.patchLen, dropout_rate = self.dropout_rate)
    ...
    def build_lstm(self):
        ...
        self.future_lstm = nn.GRU(input_size = self.hidden_layer_size,hidden_size = self.hidden_layer_size,batch_first = True,bidirectional=True)
    ...
    def build_output_head(self):
        if self.head_type == "pretrain":
            self.head = PretrainHead(self.hidden_layer_size,self.patchLen,self.input_size,self.output_size,self.dropout_rate)
    ...
    def forward(self,input,length):
        ...
        # input_embedding:[batch,patch_num,hidden,patch_len*input_size]
...
#         # out = create_patch(seq,args.patchLen)
#         out,out_mask,mask,length = patch_masking(seq,args.patchLen,0.4)
#         pred = model(out,length)
#         loss_function(pred, out,mask)
