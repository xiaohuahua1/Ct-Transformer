...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, dropout = 0, scale = True,mask_next=False, mask_diag=False):
    ...
    def forward(self, q, k, v, mask = None):
        ...
        if self.mask_next:
            if self.mask_diag:
                indices = torch.triu_indices(length, length, offset=0)
                attn[:, indices[0], indices[1]] = float('-inf')
            else:
...
class InterpretableMultiHeadAttention(nn.Module):
    def __init__(self, n_head, d_model, dropout,i):
        ...
        #     self.v_layers = nn.ModuleList([self.v_layer for _ in range(self.n_head)])
        # else:
        #     self.v_layers = nn.ModuleList([nn.Linear(self.d_model, self.d_v, bias = False) 
        #                                for _ in range(self.n_head)])
...
class attentionLayer(nn.Module):
    def __init__(self, args,i,isfc=True):
        ...
        self.self_attn_layer = InterpretableMultiHeadAttention(n_head = self.n_heads, d_model = self.hidden_layer_size,dropout = self.dropout_rate,i=i)
    ...
    def forward(self, input):
        ...
        if self.isfc:
            x_fc = self.fc(x)
            x = self.post_attn_gate_add_norm1(x_fc,x)
        return x

class Attention(nn.Module):
    def __init__(self, args,isfc = True):
        super(Attention, self).__init__()
        self.n_layers = args.n_layers
        self.layers = nn.ModuleList([attentionLayer(args,i,isfc) for i in range(self.n_layers)])
