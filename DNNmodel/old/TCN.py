...
class TemporalConvNet(nn.Module):
    def __init__(self, args,dropout=0.2):
        ...
        self.kernel_size = 2
        self.input_size = args.hidden_layer_size
        self.hidden_layer_size = args.hidden_layer_size
        self.tcnLayer = args.tcnLayer
        layers = []
        # num_levels = len(num_channels)
        for i in range(self.tcnLayer):
            dilation_size = 2 ** i   # 膨胀系数：1，2，4，8……
            # in_channels = num_inputs if i == 0 else num_channels[i-1]  # 确定每一层的输入通道数
            # out_channels = num_channels[i]  # 确定每一层的输出通道数
            in_channels = self.input_size if i == 0 else self.hidden_layer_size
            ...
            layers += [TemporalBlock(in_channels, out_channels, self.kernel_size, stride=1, dilation=dilation_size,
                                     padding=(self.kernel_size-1) * dilation_size, dropout=dropout)]
...
if __name__ == '__main__':
    ...
    model = TemporalConvNet(args).to(device)
